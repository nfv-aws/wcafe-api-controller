...
func TestSupplyUpdateOK(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	...
	serviceMock := mocks.NewMockSupplyService(ctrl)
	su := entity.Supply{Id: "cc5bafac-b35c-4852-82ca-b272cd79f2f3", Name: "dog food", Type: "food", Price: 300}
	serviceMock.EXPECT().Update(gomock.Any(), c).Return(su, nil)
	controller := SupplyController{Supplyservice: serviceMock}
	controller.Update(c)
	assert.Equal(t, http.StatusOK, c.Writer.Status())
...
func TestSupplyUpdateNotFound(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	c, _ := gin.CreateTestContext(httptest.NewRecorder())

	serviceMock := mocks.NewMockSupplyService(ctrl)
	serviceMock.EXPECT().Update(gomock.Any(), c).Return(entity.Supply{}, dynamo.ErrNotFound)
	controller := SupplyController{Supplyservice: serviceMock}
...
func TestSupplyUpdateBadRequest(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	serviceMock := mocks.NewMockSupplyService(ctrl)
	serviceMock.EXPECT().Update(gomock.Any(), c).Return(entity.Supply{}, ErrBadRequest)
	controller := SupplyController{Supplyservice: serviceMock}
